        ...
        public unsafe static Bitmap ProcessIcon(Bitmap srcImage, int resolution = 96, bool clipToCircle = true)
        {
            ...
            for (int y = 0; y < srcImage.Width; y++)
            {
                ...
                for (int x = 0; x < srcImage.Height; x++)
                {
                    ...
                    if (row[rIndex] > 0 || row[gIndex] > 0 || row[bIndex] > 0)
                    {
                        if (x > width || y > width)
                        {
                            width = Math.Max(x, y);
            ...
            for (int y = 0; y < srcImage.Width; y++)
            {
                ...
                for (int x = 0; x < srcImage.Height; x++)
                {
                    ...
                    if (row[rIndex] > 0 || row[gIndex] > 0 || row[bIndex] > 0)
                    {
                        if (x < low_gap || y < low_gap)
                        {
                            low_gap = Math.Min(x, y);
                        }
                    }
                }
            }
            ...
            for (int i = 4; i <= 8; i++)
            {
                if (width <= Math.Pow(2, i))
                {
                    high_gap = ((int)Math.Pow(2, i) - width);
                    width = (int)Math.Pow(2, i);
                    break;
                }
            }

            Bitmap tmpImage = new Bitmap(width, width, srcImage.PixelFormat);
            using (Graphics g = Graphics.FromImage(tmpImage))
            {
                RectangleF r = new RectangleF(0, 0, width, width);
                ...
                {
                    g.FillRectangle(br, 0, 0, width, width);
                }
